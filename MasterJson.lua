...
-- [🛠 Funções para salvar/carregar JSON local]
local function salvarDados()
    local json = HttpService:JSONEncode(data)
    if writefile then -- verifica se o executor suporta
        writefile("farm_data.json", json)
    else
        warn("Seu executor não suporta writefile. Dados não serão salvos localmente.")
    end
end
...
local function carregarDados()
    if isfile and isfile("farm_data.json") then
        ...
        if sucesso and type(decoded) == "table" then
            ...
            warn("Falha ao decodificar JSON local. Usando valores padrão.")
        end
    else
        print("Arquivo de dados local não encontrado, iniciando do zero.")
    end
end
...
-- Label para mudar meta mensal (clicável)
...
goalInput.FocusLost:Connect(function(enterPressed)
    if enterPressed then
        ...
        if novaMeta and novaMeta > 0 then
            ...
            goalInput.Text = tostring(data.monthlyGoal)
            print("[Auto] Valor inválido para meta mensal.")
        end
    end
end)
